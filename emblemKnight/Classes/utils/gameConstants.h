//
//  gameConstants.h
//  emblemKnight
//
//  Created by Administrator on 4/20/14.
//  Copyright (c) 2014 Administrator. All rights reserved.
//

//------------sprite related ----------//

#define spriteWidth 25.6
#define spriteHeight 28.4

#define characterScaleWidth 32
#define characterScaleHeight 32

#define battleSceneSpriteWidth 32.0
#define battleSceneSpriteHeight 32.0

#define battleSceneSpriteScaleWidth 64
#define battbleSceneSpriteScaleHeight 64

//-------------animation and delay time related ----------//
#define defaultSpriteAnimationDelayTime 0.4
#define defaultConversationBoxAnimationDuration 1.0

#define defualtBattbleSceneCharacterAnimationDelayTime 0.2

//------------- cocos2d v3 missed -----------------------//
#define kCCScrollViewSnapDuration 0.4
#define kCCScrollViewSnapDurationFallOff 100.0

//-------------- padding margin related ----------------//
#define kCommonPadding 10.0
